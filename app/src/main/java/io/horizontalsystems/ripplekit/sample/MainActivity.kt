...
private fun SetupSection(viewModel: MainViewModel) {
    Text("RippleKit Sample", style = MaterialTheme.typography.h6)
...
@Composable
private fun AccountSection(viewModel: MainViewModel, kit: RippleKit) {
    ...
    viewModel.trustLines.forEach { line ->
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Text("${RippleKit.displayCurrencyCode(line.currency)} · ${line.issuer.take(8)}…" + if (line.frozen) " (frozen)" else "")
...
private fun formatAmount(amount: Amount): String = when (amount) {
    ...
    is Amount.Issued -> "${amount.value.stripTrailingZeros().toPlainString()} ${RippleKit.displayCurrencyCode(amount.currency)}"
}
