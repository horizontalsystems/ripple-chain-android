...
class MainViewModel : ViewModel() {
    ...
    var kit by mutableStateOf<XrpKit?>(null)
        private set
    ...
    fun startFromMnemonic() {
        ...
        start {
            ...
            XrpWallet.Seed(Mnemonic().toSeed(words))
        }
    }

    fun startWatch() {
        start { XrpWallet.WatchOnly(watchAddress.trim()) }
    }

    private fun start(walletProvider: () -> XrpWallet) {
        error = null
        try {
            val wallet = walletProvider()
            val kit = XrpKit.getInstance(App.instance, wallet, network, walletId = "sample-${network.name}")
            this.kit = kit
            observe(kit)
            kit.start()
        } catch (e: Exception) {
    ...
    private fun observe(kit: XrpKit) {
