...
internal class SyncTimer(
    ...
    var state: State = State.NotReady(RippleKit.SyncError.NotStarted())
        private set(value) {
            if (value != field) {
                field = value
    ...
    fun stop() {
        ...
        state = State.NotReady(RippleKit.SyncError.NotStarted())
        scope = null
        stopTimer()
    }
    ...
    private fun handleConnectionChange() {
        ...
        if (connectionManager.isConnected) {
            ...
            state = State.NotReady(RippleKit.SyncError.NoNetworkConnection())
            stopTimer()
        }
    }
